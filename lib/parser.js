...
module.exports = {
  ...
  formatWHISPER(event) {
    const parsed = {}
    const msg_parts = event.split('WHISPER ')[1]
    let split_msg_parts = msg_parts.split(' :')
    const channel = split_msg_parts[0]
    ...
    let [tags,username] = event.split('WHISPER')[0].split(' :')
    parsed.username = username.split('!')[0]
    ...
    // Wispers go over a 1 to 1 groupchat, the channelname should represent this.
    // The prefix WHISPER should minimalize the chance of the channel existing
    parsed.channel = `WHISPER_${channel}_${username}`
    parsed.message = message
